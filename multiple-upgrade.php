...
// Web request upgrade
if (!IN_CLI) {
   die('Error: upgrade from browser not allowed');
} else {
    // CLI upgrade
...
{
    $dirs = glob(MAUTIC_APP_ROOT.'/cache*', GLOB_ONLYDIR);
    foreach ($dirs as $dir) {
        $cacheDirProd = $dir.'/prod';
        if (!recursive_remove_directory($cacheDirProd)) {
            process_error_log(['Could not remove the application cache.  You will need to manually delete '.$cacheDirProd.'.']);
...
{
    // Rebuild the cache
    return run_symfony_command('mautic:multiple:run', ['--command="cache:clear --no-interaction --no-warmup --env=prod --no-debug"']);
}
...
function apply_critical_migrations()
{
    ...
    if ($criticalMigrations) {
        foreach ($criticalMigrations as $version) {
            if (!run_symfony_command('mautic:multiple:run', ['--command="doctrine:migrations:migrate '.$version.' --no-interaction --env=prod --no-debug"'])) {
                $success = false;
            }
        }
    }
